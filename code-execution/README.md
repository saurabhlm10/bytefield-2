...
```bash
code-execution$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
code-execution$ sam local start-api
code-execution$ curl http://localhost:3000/
```
...
```bash
code-execution$ sam logs -n HelloWorldFunction --stack-name code-execution --tail
```
...
```bash
sam delete --stack-name code-execution
```
